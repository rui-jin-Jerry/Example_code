...
%Par.channelLocationsDirectory = 'R:\Code and EEG lab\labmembers-brisc-eeg-4d0c72fdf9df\Preprocessing Code\Channel Locations File';
...
Par.highPassFilterCutoff_ForChecking = 1;
...
for datasetNo = subjectIDsToDo
    ...
    EEG = pop_fileio([Par.dataFolder_Raw, '/' Par.subjectCodesList{datasetNo}, '/', Par.subjectCodesList{datasetNo}, '_', Par.testingPhase, '_', Par.experimentName, '.eeg']);
    ...
    % File used in the next script (not yet filtered or re-referenced)
    ...
    elseif strcmp(Par.referenceOption_ForChecking, 'TP10') % If using a TP10 reference
        
        EEG = pop_reref(EEG, find(strcmp(channelLabels, 'TP10')), 'exclude', Par.movementSensorChanInds);
        
    elseif strcmp(Par.referenceOption_ForChecking, 'Pz') % If using a Pz reference
    ...
    % Visualize spectrum/raw eeg signal
    % pop_spectopo(EEG)
    % pop_eegplot(EEG)
